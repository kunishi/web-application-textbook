...
2. 送り手は、$$K$$を用いて送信したいデータ$$D$$を暗号化し、暗号化後のデータ$$D'$$を受け手に送信する。
$$
D' = {encrypt}（D, K)
$$
3. 受け手は、$$K$$を用いて$$D'$$を復号し、元のデータ$$D$$を得る。
...
公開鍵暗号方式の性質により、$$D = encrypt(K, PK_{serv})$$の復号は$$SK_{serv}$$が必要であり、（Webサーバで$$SK_{serv}$$が厳格に保管されている限り）盗聴される心配はないことに注意してほしい。
