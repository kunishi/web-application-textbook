...
2. Webクライアント`client`は共通鍵$$K$$を生成し、次に$$PK_{serv}$$をサーバから取得し（`Server Certificate`）、$$PK_{serv}$$で$$K$$を暗号化する（$$D = encrypt(K, PK_{serv})$$）。暗号化した結果$$D$$を`server`に送信する。（`Client Key Exchange`）
...
改ざんへの対策にはダイジェストが用いられる。ただし、通常の暗号学的ハッシュ関数ではなく、送信したいデータと共通鍵を用いて計算を行うため、**メッセージ認証符号**（Message Authentication Code, MAC）と呼ばれている。
...
1. Webクライアント`client`は、自身が利用できる暗号スイートの一覧をWebサーバ`server`に送信する。(`Client Hello`)
2. `server`は、`client`から送られてきた暗号スイートの中から一つを選び、`client`に送信する。(`Server Hello`)