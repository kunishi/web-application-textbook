...
2. Webクライアント`client`は共通鍵$$K$$を生成し、次に$$PK_{serv}$$をサーバから取得し（`ServerCertificate`メッセージ）、$$PK_{serv}$$で$$K$$を暗号化する（$$D = encrypt(K, PK_{serv})$$）。暗号化した結果$$D$$を`server`に送信する。（`ClientKeyExchange`メッセージ）
