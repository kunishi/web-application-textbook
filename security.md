...
[^2r]: 厳密には、TLSで使用できる公開鍵暗号方式にはいくつか選択肢があること、TLSで使用される共通鍵は複数であることなどの理由で、共通鍵を直接送るのではなく、プリマスターシークレットというデータを共有したのち、クライアント・サーバで合意されたアルゴリズムに従いマスターシークレットというデータを（サーバとクライアント双方で）生成、さらに必要な数だけ共通鍵を（サーバとクライアント双方で）生成、という手順を取る。(出典: http://detail.chiebukuro.yahoo.co.jp/qa/question_detail/q10142993040)
