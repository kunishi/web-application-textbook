...
1. 送信側で、送信したいデータ$$D$$からダイジェスト$$H=hash(D)$$を計算し、$$D$$と$$H$$をまとめて共通鍵$$K$$で暗号化（$$D' = encrypt(\{D, H\}, K)$$）、$$D'$$を送信する。
2. 受信側では$$D'$$から$$D$$と$$H$$を取り出し（$$\{D, H\} = decrypt(D', K)$$）、$$D$$のダイジェストを計算する（$$H' = hash(D)$$）。一方向ハッシュ関数の性質から、$$H = H'$$なら通信路上での改ざんはないと保証することができる。
...
3. `CA`は$$PK_{serv}$$を$$SK_{CA}$$で暗号化（$$C = encrypt(PK_{serv}, SK_{CA})$$）する。この$$C$$が$$PK_{serv}$$のデジタル公開鍵証明書になる。