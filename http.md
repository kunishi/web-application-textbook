...
* 主なHTTPメソッド
  ...
  * POST: 指定したURIに対する子資源の作成など
    ...
    * 例：`POST http://blog.example.com/` … ブログ[http:\/\/blog.example.com\/](http://blog.example.com/) に新しい投稿という子資源を作成する

  * PUT: 資源の更新

    * 例：`PUT http://blog.example.com/1` … ブログの投稿 [http:\/\/blog.example.com\/1](http://blog.example.com/1) の修正版を投稿する

  * DELETE: 資源の削除

    * 例：`DELETE http://blog.example.com/1` …ブログの投稿 [http:\/\/blog.example.com\/1](http://blog.example.com/1) を削除する
