...
`function() {...}`という記述に着目してほしい。これは関数の呼び出しではない。プログラム中で処理可能な値[^1]としての関数、平たく言えば「関数」という種類の値（第一級関数、first-class function）である。したがって、`$('#menu li').hover({function() {...}, function() {...});`は、マウスを当てると第1引数の値（＝関数）を実行し、マウスを外すと第2引数の値（＝関数）を実行する、という関数になる。
